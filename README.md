...
* **지원 운영체제**: 
  ...
  * **macOS**: 지원 (macOS 11.0 이상, Apple Silicon)
...
* **키프레임 불필요**: 클립 또는 조정 레이어의 시작 지점(컷편집 분할 지점)에서 효과가 0프레임부터 자동으로 시작되며, 설정한 지속 시간 동안 화면 이동이 진행됩니다. 클립에 전환 효과를 적용한 경우, 전환 효과가 시작되는 시점이 0프레임으로 설정됩니다.
...
효과 컨트롤 패널 내에 위치한 버튼형 UI를 통해 사전에 설정된 값으로 화면 이동 지정할 수 있습니다.
...
#### 업데이트 알림
* 효과 컨트롤을 열면 GitHub의 최신 정식 릴리스 버전을 확인합니다.
* 새 버전이 있을 때만 최상단에 GitHub 릴리스 페이지로 연결되는 버튼이 표시됩니다. 설치는 사용자가 직접 진행합니다.


## 설치 방법

### 1. 패키지 설치
* **Windows**: 우측 **Releases** 탭에서 최신 Windows용 설치 파일을 다운로드하여 실행하면 Adobe 공용 플러그인 폴더에 자동 설치됩니다.
