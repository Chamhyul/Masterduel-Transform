...
bool DXContext::LoadShaderFromMemory(
    ...
    ShaderObjectPtr &outShaderObject)
{
    if (!inCSOData || inCSOSize == 0 || !inRSData || inRSSize == 0) { return false; }

    HRESULT res = S_OK;

    // Create Root Signature directly from memory
    res = mDevice->CreateRootSignature(
        0,
        inRSData,
        inRSSize,
        IID_PPV_ARGS(&outShaderObject->mRootSignature));
    if (FAILED(res)) { return false; }

    // Create Pipeline State directly from memory
    D3D12_COMPUTE_PIPELINE_STATE_DESC pipelineStateDesc = {};
    pipelineStateDesc.pRootSignature = outShaderObject->mRootSignature.Get();
    pipelineStateDesc.CS.pShaderBytecode = inCSOData;
    pipelineStateDesc.CS.BytecodeLength = inCSOSize;
    res = mDevice->CreateComputePipelineState(&pipelineStateDesc, IID_PPV_ARGS(&outShaderObject->mPipelineState));
    if (FAILED(res)) { return false; }
    ...
    UINT inParamBufferSize)
{
    ...
    // Align size to 256 bytes for Constant Buffer requirements
    UINT alignedSize = (inParamBufferSize + 255) & ~255;
    ...
    if (alignedSize > inParamBufferSize)
    {
        memset((uint8_t*)mappedMemory + inParamBufferSize, 0, alignedSize - inParamBufferSize);
    }
