...
2. **비공개 / 미존재 프로퍼티 키 사용 금지**:
   - 접두어 없는 `EffectiveTrackItemStartAsTicks`를 추측해 조회하지 말 것. `ClipNode::EffectiveTrackItemStartAsTicks`는 2026-09-25 V2 로그의 실제 Clip 노드에서 확인됨.
   - 과거 환경에서는 `EffectNode::RuntimeInstanceID` 조회에 실패했으나, 2026-09-25 V2 로그에서는 현재 효과와 트리 오퍼레이터 양쪽에 실재함을 확인. 확인된 환경의 소속 매칭에 사용하며, 부재/파싱 실패 시 다른 레이어를 임의 선택하지 말 것. 미실측 키를 추가로 추측하지 말 것.
...
### [2026-09-25] 뒤쪽 전환에서 효과 재시작 — 진단 단계
- 사용자 요구: 비디오/조정 레이어 모두 클립 시작에서 효과 시작. 시작 전환이 있으면 전환 시작에서 효과 시작. 끝 전환은 기존 효과 시간에 영향을 주지 않아야 함.
- 사용자가 끝 전환에서 효과가 재시작하는 현상을 보고함. 원인은 아직 로그로 확정하지 않음.
- `AT_TIMING_DIAGNOSTICS=1 bash AutoTransform/Mac/build.sh`로 Metal 진단 빌드 생성. 기본 빌드는 로그 비활성화.
- 로그: `/tmp/masterduel-timing-<Premiere 프로세스 ID>.log`. 효과 ID, 입력/오퍼레이터 노드 구조, 선택 레이어, InPoint, 전환 보정, 캐시, 최종 프레임을 기록. ownerNodeID가 0이어도 기록함.
- 파일당 32 MiB에서 기록 중단. 프레임별 트리 탐색은 깊이 8 및 노드 128개로 제한. 진단 빌드에서는 성능 저하가 있을 수 있음.
- 이번 단계는 로깅만 추가하며 타이밍 선택/계산은 변경하지 않음. 재현 로그 확인 후 원인 수정 진행.

### [2026-09-25] 진단 V1 분석 및 V2 식별 정보 보강
- 실측 로그 `masterduel-timing-94859.log`: render 567까지 frame=106, 다음 시퀀스 프레임의 render 568/569에서 서로 다른 effect=7778/7781이 모두 transition input index=1을 선택하고 frame=0으로 계산됨. 이 구간의 head/cacheHead는 0.
- 노드 ID는 같은 구조를 재조회해도 달라짐. 현재 효과 ID와 트리의 오퍼레이터 ID가 같을 것이라 가정해 수정하지 말 것.
- V2는 GetNodeInfo의 해시 및 IterateNodeProperties가 실제 반환한 속성 목록을 기록. 시간/해시/인스턴스 관련 값만 제한적으로 출력하며 파일명, 클립명, opaque data 값은 기록하지 않음.
- V2 역시 진단 전용. 해시가 효과 소속을 유일하게 식별하는지 미확인이고, 타이밍 선택/계산은 변경하지 않음.

### [2026-09-25] 끝 전환 재시작 수정 — Mac 검증 빌드 V3
- V2 실측: 현재 효과 66660은 전환 입력 0, 효과 66668은 입력 1 소속. 기존 코드는 둘 다 입력 1을 선택. 노드 핸들은 재조회 시 바뀌지만 RuntimeInstanceID 및 효과 해시는 연결됨.
- 조정 레이어 직속 Clip의 오퍼레이터에서 현재 효과 RuntimeInstanceID를 확인한 경우에만 해당 레이어 선택. 배경 컴포지터 내부의 효과를 소속으로 오인하지 않도록 직속 Clip만 검사.
- 전환 입력 0은 해당 끝 전환으로부터 시작 보정을 유도하지 않음. 입력 1에만 기존 시작 전환 보정을 적용. 근거 없는 전환 길이 절반 보조 계산 제거.
- 시작 전환 역탐색에도 동일한 소속 조건 적용. 이전 캐시를 읽어 재사용하지 않고 현재 시작 전환을 재조회하여 변경/삭제 후 이전 값이 남지 않도록 함.
- 실제 C++ 선택 함수를 SDK 모의 호스트에서 V2 로그로 재생: 조정 레이어 374건의 InPoint 소속 선택, 미일치 식별값 거부, 노드 해제 균형 통과. 문제 render 39는 107프레임, render 40은 0프레임 확인. AddressSanitizer/UBSan 적용.
- Windows 코드는 아직 변경하지 않음. Mac 호스트에서 사용자 재현 검증 후 이식. V3도 진단 로그 유지; 실사용 해결 확정 전.

### [2026-09-25] v0.1.3 Mac 배포 빌드
- 사용자 실사용 확인: 시작 전환은 전환 시작에서 효과 시작, 끝 전환은 기존 진행을 유지.
- V3에서 효과 소속 판정과 시작 전환 역탐색을 보존하고 진단 로그/트리 전수 탐색 코드를 제거. 버전/패키지 메타데이터를 0.1.3으로 동기화.
- Windows 소스의 타이밍 수정 및 Windows용 설치 파일 빌드는 별도 Windows 환경에서 검증 필요. macOS에서는 MSVC/CUDA/Windows Adobe SDK 기반 `.aex`와 Inno Setup `.exe`를 생성할 수 없음.
